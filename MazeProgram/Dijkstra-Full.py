...
pygame.display.set_caption("dijkstra full")
...
def randomNeighbours(grid, current):
    ...
    # This is checking whether each neighbouring cell has been visited
    # If not, add it to the neighbours list to be randomly selected
    if (not current.walls[0]):
        neighbours.append(top)
        
    if (not current.walls[1]):
        neighbours.append(right)
        
    if (not current.walls[2]):
        neighbours.append(bottom)
        
    if (not current.walls[3]):
        neighbours.append(left)
    ...
    # Solving Algorithm / Solving the Maze            
        ...
        if (not allNodesChecked):
            ...
            if (solving_current != next_cell):
                ...
                nearbyCells = randomNeighbours(grid, next_cell)
                lowestWeightedCell = nearbyCells[0]
                for i in range(0, len(nearbyCells)):
                    if (lowestWeightedCell.weight < nearbyCells[i].weight):
                        lowestWeightedCell = nearbyCells[i]
            ...
            print("solved using dijkstra full")
            print("time:",time,"seconds")
            print("full count:",full_count)
            print("count:",count)
            f = open("dijsktra-full-output-file.txt", "w")
            f.write(str(time) + "\n")
